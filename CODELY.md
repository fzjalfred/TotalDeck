...
- [2026-08-30 22:09:11] TotalDeck 地图与玩家分离架构（2026-08-30 重构定稿）：MapDef 类定义地图（名称/Ground 尺寸/圈心圈径/spawnPoints[] 出生槽数组），MapDef.Available() 供菜单枚举，加新地图只需加一个 MapDef；SpawnAssignment 绑定势力→槽位；GameManager.StartNewGame(map, playerAssign, enemyAssign) 接收菜单选择，ApplyMapGeometry() 把几何推给场景（Ground scale/HillZone 圈/分界线/相机），IsInDeployZone 按出生槽位相对圈心方向判定。地图翻倍尺寸：Ground 100×100（scale 10,1,10）、出生点 z=±30、AI 增援 z=-40、分界线 80、相机高 70 平移 ±40、圈 r=16（GameConfig.HillRadius）。占山为王：HillZone 每帧统计圈内士兵，10s 结算多数方 +1，100 分胜；计分只在 Combat 走表，Planning 冻结且进度条清空；HillScoreUI 计分板【分数|圈内人数】蓝左红右 + 底部进度条锚点拉伸实现。AIController 每 0.5s 花光钱（抽卡→法术→部署最贵兵，站位从自己出生槽位方向推导），战斗阶段 idle 军团走向圈，圈上有敌军团优先攻击。游戏流程：GameState 枚举（MainMenu/Playing/GameOver）+ GameMenuUI 三面板（MainMenuPanel 含地图/出生点下拉、GameOverPanel 含比分与击杀阵亡统计、HUDRoot 归组）；SideStats 记每方击杀/阵亡，Soldier.TakeDamage 加 attacker 参数归因；BattleInitializer 已废弃为空壳（出生逻辑归 GameManager）。**Why:** 用户要求地图与玩家解耦、可选地图可分配出生点、菜单框架可扩展、结算统计杀/亡；AI 曾因缓存 PlayerState 引用在重开后不买兵。**How to apply:** 加新地图只加 MapDef；改经济/规则改 PlayerState 一处双方生效；AI 永不缓存会话级引用；UI 进度条一律锚点拉伸；加新菜单项改 GameMenuUI+TotalDeckSceneSetup.CreateMenuFramework 两处。
